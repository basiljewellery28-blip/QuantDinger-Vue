...
const locale = {
  ...
  'trading-bot.type.vpin_toxic_scalper': 'VPIN Toxic Scalper',
  'trading-bot.type.vpin_toxic_scalperDesc': 'Avellaneda-Stoikov market maker with VPIN-based toxic-flow detection. L2 LOB aware.',
  'trading-bot.type.vpin_toxic_scalperHint': 'Tune AS bands (gamma/kappa) and VPIN toxicity threshold. Defaults work for EURUSD-scale FX pairs.',
  'trading-bot.scene.hft': 'HFT / market-making',
  'trading-bot.vpin.aboutTitle': 'About this strategy',
  'trading-bot.vpin.aboutDesc': 'Optimal market-making (Avellaneda-Stoikov) with two risk overlays: VPIN-based toxic-flow detection (halves size when informed-trader flow dominates) and ATR-spike guard (halves size on sudden vol bursts). Reads Level 2 LOB via MT5 market book polling (or WebSocket feed if configured).',
  'trading-bot.vpin.asTitle': 'Avellaneda-Stoikov spread',
  'trading-bot.vpin.vpinTitle': 'VPIN toxicity detector',
  'trading-bot.vpin.executionTitle': 'Execution parameters',
  'trading-bot.vpin.riskAversion': 'Risk aversion (gamma)',
  'trading-bot.vpin.riskAversionHint': 'Inventory aversion coefficient. Higher = quotes shift more aggressively toward flat. Try 50 for EURUSD-scale, 0.5 for JPY pairs, 0.05 for XAUUSD.',
  'trading-bot.vpin.riskAversionReq': 'Risk aversion is required',
  'trading-bot.vpin.bookLiquidity': 'Book liquidity (kappa)',
  'trading-bot.vpin.bookLiquidityHint': 'Effective book density. Together with gamma sets quote width via (1/gamma)·ln(1 + gamma/kappa). Try 12500 for EURUSD-scale, 125 for JPY, 12 for XAUUSD.',
  'trading-bot.vpin.bookLiquidityReq': 'Book liquidity is required',
  'trading-bot.vpin.spreadPreview': 'Quote half-spread at current params',
  'trading-bot.vpin.vpinThreshold': 'VPIN CDF threshold',
  'trading-bot.vpin.vpinThresholdHint': 'When rolling VPIN percentile exceeds this, order size is halved (0.8 = top 20% of volume buckets flagged as toxic).',
  'trading-bot.vpin.vpinThresholdReq': 'VPIN threshold is required',
  'trading-bot.vpin.bucketVolume': 'Volume bucket size',
  'trading-bot.vpin.bucketVolumeHint': 'Volume per VPIN bucket (V in the formula). Smaller = more reactive but noisier. 1000 is the typical default.',
  'trading-bot.vpin.bucketVolumeReq': 'Bucket volume is required',
  'trading-bot.vpin.rollingWindow': 'Rolling buckets (N)',
  'trading-bot.vpin.rollingWindowHint': 'How many recent buckets feed the rolling VPIN. 50 is the Easley/Lopez-de-Prado default.',
  'trading-bot.vpin.rollingWindowReq': 'Rolling window is required',
  'trading-bot.vpin.orderSize': 'Order size (lots)',
  'trading-bot.vpin.orderSizeHint': 'MT5 lots per entry. 0.01 = 1 micro lot, ~$1k notional on EURUSD. Halved automatically when VPIN or ATR guard triggers.',
  'trading-bot.vpin.orderSizeReq': 'Order size is required',
  'trading-bot.vpin.maxInventory': 'Max inventory (lots)',
  'trading-bot.vpin.maxInventoryHint': 'Total directional exposure cap in MT5 lots. Strategy stops adding once net |position| hits this. Keep tight on demo (e.g. 1.0).',
  'trading-bot.vpin.maxInventoryReq': 'Max inventory is required',
  'trading-bot.vpin.volatilityPeriod': 'Volatility window (bars)',
  'trading-bot.vpin.volatilityPeriodHint': 'Rolling window for sigma in the reservation-price formula. 20 bars is typical.',
  'trading-bot.vpin.volatilityPeriodReq': 'Volatility period is required',
  'trading-bot.vpin.validateFailed': 'Please fix the VPIN config fields highlighted above',
  'trading-bot.vpin.fetchScriptFailed': 'Failed to fetch the VPIN strategy script from the backend',
  // Pairs Trader (cointegration + Kalman hedge ratio)
  'trading-bot.type.pairs_trader': 'Pairs Trader',
  'trading-bot.type.pairs_traderDesc': 'Cointegration / mean-reversion bot. Trades the primary leg against a Kalman-updated hedge ratio of a second symbol.',
  'trading-bot.type.pairs_traderHint': 'Pick a related symbol (e.g. XAUUSD vs XAGUSD). Strategy goes counter to spread z-score and uses OU half-life for the time stop.',
  'trading-bot.pairs.aboutTitle': 'About this strategy',
  'trading-bot.pairs.aboutDesc': 'Cointegration mean-reversion: primary symbol is hedged 1:beta against a second symbol, where beta is updated online via a scalar Kalman filter. Enters on spread z-score blow-outs, exits on reversion, and emergency-flattens above the stop-z threshold. The offline estimator (scripts/pairs_offline_estimator.py) periodically refreshes the Kalman priors from longer history.',
  'trading-bot.pairs.pairTitle': 'Pair definition',
  'trading-bot.pairs.symbolB': 'Hedge symbol (B)',
  'trading-bot.pairs.symbolBHint': 'The second leg used for hedging. The primary leg (above) is symbol A. Common pairs: XAUUSD/XAGUSD, EURUSD/GBPUSD.',
  'trading-bot.pairs.symbolBReq': 'Hedge symbol is required',
  'trading-bot.pairs.orderSize': 'Order size (lots)',
  'trading-bot.pairs.orderSizeHint': 'MT5 lots per entry on the primary leg. 0.01 = 1 micro lot. Hedge leg is sized via beta when hedgeSizeMode=beta.',
  'trading-bot.pairs.orderSizeReq': 'Order size is required',
  'trading-bot.pairs.maxInventory': 'Max inventory (primary)',
  'trading-bot.pairs.spreadTitle': 'Spread statistics',
  'trading-bot.pairs.spreadWindow': 'Spread window (bars)',
  'trading-bot.pairs.spreadWindowHint': 'Rolling bars used to compute spread mean/std and z-score. 120 bars is typical for intraday work.',
  'trading-bot.pairs.entryZ': 'Entry |z|',
  'trading-bot.pairs.entryZReq': 'Entry z is required',
  'trading-bot.pairs.exitZ': 'Exit |z|',
  'trading-bot.pairs.stopZ': 'Stop |z|',
  'trading-bot.pairs.stopZHint': 'Emergency flatten when |spread z-score| exceeds this. Catches spread blow-outs that invalidate the cointegration assumption.',
  'trading-bot.pairs.stopZReq': 'Stop z is required',
  'trading-bot.pairs.kalmanTitle': 'Kalman filter (online hedge ratio)',
  'trading-bot.pairs.kalmanInitBeta': 'Initial beta',
  'trading-bot.pairs.kalmanInitBetaHint': 'Prior mean for the hedge ratio. The offline estimator overrides this when run; for first-time setup, set to your best OLS guess.',
  'trading-bot.pairs.kalmanProcessVar': 'Process variance Q',
  'trading-bot.pairs.kalmanProcessVarHint': 'How fast beta is allowed to drift per bar. 1e-5 is conservative; raise for faster adaptation, lower for stability.',
  'trading-bot.pairs.riskTitle': 'Risk + adaptive time stop',
  'trading-bot.pairs.ouWindow': 'OU half-life window (bars)',
  'trading-bot.pairs.maxHoldBarsCap': 'Max hold cap (bars)',
  'trading-bot.pairs.maxHoldBarsCapHint': 'Hard cap on time-in-position. Effective max-hold = min(this, 3 * OU half-life), so faster reversion = earlier time stop.',
  'trading-bot.pairs.validateFailed': 'Please fix the Pairs config fields highlighted above',
  'trading-bot.pairs.fetchScriptFailed': 'Failed to fetch the Pairs Trader script from the backend',
  // BotDetail HFT-metrics panel (VPIN/OFI/AS bands card group). vue-i18n returns
  // the raw key when missing, so the `|| 'Fallback'` JS guards never trigger.
  'trading-bot.hft.title': 'HFT Real-Time Metrics',
  'trading-bot.hft.minimize': 'Minimize',
  'trading-bot.hft.expand': 'Expand',
  ...
  'trading-bot.kpi.accountEquity': 'Account Equity',
  'trading-bot.kpi.allocated': 'Allocated / Free',
  ...
  'trading-bot.wizard.savedCredential': 'Broker Account',
